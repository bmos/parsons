...
class SolidarityTechTaskAgents(SolidarityTechBase):
    ...
    def get_task_agents(
        ...
        task_id is not None and params.update({"task_id": task_id})
        ...
        expected_responses = {HTTPStatus.OK: (True, "task agents listed")}
    ...
    def get_task_agent(
        ...
        expected_responses = {
            HTTPStatus.OK: (True, "task agent found"),
            HTTPStatus.NOT_FOUND: (False, "task agent not found"),
        }
        self._handle_status_codes(res=res, codes=expected_responses)
    ...
    def create_task_agent(
        ...
        """
        payload: dict[str, _JsonType] = {"user_id": user_id, "task_id": task_id}

        res = self._post_request(
            "task_agents", payload=payload, additional_headers={"content-type": "application/json"}
        )

        expected_responses = {HTTPStatus.CREATED: (True, "task agent created")}
    ...
    def delete_task_agent(
        ...
        expected_responses = {HTTPStatus.NOT_FOUND: (False, "task agent not found")}
        return self._handle_status_codes(res=res, codes=expected_responses)
