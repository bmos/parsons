...
class SolidarityTechUserRelationships(SolidarityTechBase):
    ...
    def get_user_relationships(
        ...
        expected_responses = {
            HTTPStatus.OK: (True, "user relationships listed"),
        }
        self._handle_status_codes(res=res, codes=expected_responses)

        data: list[UserRelationshipData] = res.json()
        return Table(data, name="Solidarity Tech User Relationships")
    ...
    def create_user_relationship(
        ...
        expected_responses = {
            HTTPStatus.CREATED: (True, "user relationship created"),
            HTTPStatus.UNPROCESSABLE_ENTITY: (False, "invalid request"),
        }
    ...
    def delete_user_relationship(
        ...
        expected_responses = {
            HTTPStatus.OK: (True, "user relationship deleted"),
            HTTPStatus.NOT_FOUND: (False, "user relationship not found"),
        }
        return self._handle_status_codes(res=res, codes=expected_responses)
