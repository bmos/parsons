...
if TYPE_CHECKING:
    from parsons.solidarity_tech.datatypes import SentEmailData
...
class SolidarityTechEmails(SolidarityTechBase):
    ...
    def send_one_off_email(
        ...
        *,
        ...
        body_plain is not None and params.update({"body_plain": body_plain})
        email_sender_id is not None and params.update({"email_sender_id": email_sender_id})
        reply_to is not None and params.update({"reply_to": reply_to})
        attachment_urls is not None and params.update({"attachment_urls": attachment_urls})

        res = self._post_request("emails", params=params)

        expected_responses = {
            HTTPStatus.CREATED: (True, "email sent successfully"),
