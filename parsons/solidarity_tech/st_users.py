...
class SolidarityTechUsers(SolidarityTechBase):
    ...
    def get_users(
        ...
        user_list_ids is not None and params.update({"user_list_ids": user_list_ids})
        phone_number is not None and params.update({"phone_number": phone_number})
        email is not None and params.update({"email": email})
        ...
        expected_responses = {
            HTTPStatus.OK: (True, "users listed"),
            HTTPStatus.UNPROCESSABLE_ENTITY: (False, "invalid user filter"),
        }
        ...
        return Table(data, name="Solidarity Tech Users"), meta
    ...
    def get_user(
        ...
        expected_responses = {
            HTTPStatus.OK: (True, "user found"),
            HTTPStatus.NOT_FOUND: (False, "user not found"),
        }
        self._handle_status_codes(res=res, codes=expected_responses)
    ...
    def create_user(
        ...
        *,
        ...
        phone_number is not None and payload.update({"phone_number": phone_number})
        email is not None and payload.update({"email": email})
        first_name is not None and payload.update({"first_name": first_name})
        last_name is not None and payload.update({"last_name": last_name})
        alternate_name is not None and payload.update({"alternate_name": alternate_name})
        preferred_language is not None and payload.update(
            {"preferred_language": preferred_language}
        )
        second_language is not None and payload.update({"second_language": second_language})
        chapter_id is not None and payload.update({"chapter_id": chapter_id})
        chapter_ids is not None and payload.update({"chapter_ids": chapter_ids})
        referred_by_user_id is not None and payload.update(
            {"referred_by_user_id": referred_by_user_id}
        )
        custom_user_properties is not None and payload.update(
            {"custom_user_properties": custom_user_properties}
        )
        add_tags is not None and payload.update({"add_tags": add_tags})
        remove_tags is not None and payload.update({"remove_tags": remove_tags})
        donation_charge is not None and payload.update({"donation_charge": donation_charge})
        address is not None and payload.update({"address": address})
        assessment is not None and payload.update({"assessment": assessment})
        sms_permission is not None and payload.update({"sms_permission": sms_permission})
        call_permission is not None and payload.update({"call_permission": call_permission})
        email_permission is not None and payload.update({"email_permission": email_permission})
        timezone is not None and payload.update({"timezone": timezone})
        lookup_key is not None and payload.update({"lookup_key": lookup_key})
        ...
        expected_responses = {
            HTTPStatus.OK: (True, "user updated via lookup_key - existing user found"),
            HTTPStatus.CREATED: (True, "user created with lookup_key - new user"),
            HTTPStatus.FORBIDDEN: (False, "multi-chapter feature not enabled"),
            HTTPStatus.UNPROCESSABLE_ENTITY: (
                False,
                "provided lookup_key without value in custom_user_properties",
            ),
        }
    ...
    def update_user(
        ...
        *,
        ...
        phone_number is not None and payload.update({"phone_number": phone_number})
        clear_phone_number is not None and payload.update(
        ...
        chapter_id is not None and payload.update({"chapter_id": chapter_id})
        chapter_ids is not None and payload.update({"chapter_ids": chapter_ids})
        add_chapter_ids is not None and payload.update({"add_chapter_ids": add_chapter_ids})
        remove_chapter_ids is not None and payload.update(
            {"remove_chapter_ids": remove_chapter_ids}
        )
        set_exclusive_chapter is not None and payload.update(
        ...
        expected_responses = {
            HTTPStatus.OK: (True, "user updated"),
            HTTPStatus.UNPROCESSABLE_ENTITY: (
                False,
    ...
    def merge_duplicate_users(
        ...
        expected_responses = {
            HTTPStatus.OK: (True, "users merged successfully"),
            HTTPStatus.NOT_FOUND: (False, "user not found"),
            HTTPStatus.UNPROCESSABLE_ENTITY: (False, "invalid parameters"),
        }
        self._handle_status_codes(res=res, codes=expected_responses)
    ...
    def delete_user(
        ...
        expected_responses = {
            HTTPStatus.OK: (True, "user deleted"),
            HTTPStatus.NOT_FOUND: (False, "user not found"),
        }
        self._handle_status_codes(res=res, codes=expected_responses)
