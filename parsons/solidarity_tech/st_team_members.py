...
if TYPE_CHECKING:
    ...
    from parsons.solidarity_tech.datatypes import InviteType, ScopeType
...
class SolidarityTechTeamMembers(SolidarityTechBase):
    ...
    def get_team_members(
        ...
        expected_responses = {HTTPStatus.OK: (True, "team members listed")}
    ...
    def create_team_member(
        ...
        member_id is not None and payload.update({"member_id": member_id})
        phone_number is not None and payload.update({"phone_number": phone_number})
        email is not None and payload.update({"email": email})
        full_name is not None and payload.update({"full_name": full_name})
        ...
        task_id is not None and payload.update({"task_id": task_id})
        ...
        expected_responses = {
            HTTPStatus.CREATED: (True, "team member created"),
    ...
    def update_team_member(
        ...
        expected_responses = {HTTPStatus.OK: (True, "team member updated")}
        return self._handle_status_codes(res=res, codes=expected_responses)
