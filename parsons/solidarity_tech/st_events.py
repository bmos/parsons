...
class SolidarityTechEvents(SolidarityTechBase):
    ...
    def get_events(
        ...
        scope_id is not None and params.update({"scope_id": scope_id})
        scope_type is not None and params.update({"scope_type": scope_type})
        ...
        return Table(data, name="Solidarity Tech Events")
    ...
    def create_event(
        ...
        *,
        ...
        virtual_url is not None and payload.update({"virtual_url": virtual_url})
        location_name is not None and payload.update({"location_name": location_name})
        session_title is not None and payload.update({"session_title": session_title})
        tags is not None and payload.update({"tags": tags})
        max_capacity is not None and payload.update({"max_capacity": max_capacity})
        latitude is not None and payload.update({"latitude": latitude})
        longitude is not None and payload.update({"longitude": longitude})
        skip_duplicate_check is not None and payload.update(
            {"skip_duplicate_check": skip_duplicate_check}
        )
        allow_long_title is not None and payload.update({"allow_long_title": allow_long_title})
        automated_communications is not None and payload.update(
