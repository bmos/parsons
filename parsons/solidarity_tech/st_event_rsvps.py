...
class SolidarityTechEventRSVPs(SolidarityTechBase):
    ...
    def get_event_rsvps(
        ...
        *,
        ...
        event_id is not None and params.update({"event_id": event_id})
        session_id is not None and params.update({"session_id": session_id})
    ...
    def create_event_rsvp(
        ...
        *,
        ...
        user_id is not None and payload.update({"user_id": user_id})
        is_confirmed is not None and payload.update({"is_confirmed": is_confirmed})
        source is not None and payload.update({"source": source})
        source_system is not None and payload.update({"source_system": source_system})
    ...
    def update_event_rsvp(
        ...
        *,
        ...
        """
        payload: dict[str, _JsonType] = {}
        is_attending is not None and payload.update({"is_attending": is_attending})
        is_confirmed is not None and payload.update({"is_confirmed": is_confirmed})
        agent_user_id is not None and payload.update({"agent_user_id": agent_user_id})
        source is not None and payload.update({"source": source})
        source_system is not None and payload.update({"source_system": source_system})
