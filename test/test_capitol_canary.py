...
class TestP2A(unittest.TestCase):
    ...
    def test_update_advocate(self, m):
        ...
        # Test arg validation - sms opt in requires a phone
        pytest.raises(ValueError, lambda: self.cc.update_advocate(advocate_id=1, sms_optin=True))

        # Test arg validation - email opt in requires a email
        pytest.raises(ValueError, lambda: self.cc.update_advocate(advocate_id=1, email_optin=True))
