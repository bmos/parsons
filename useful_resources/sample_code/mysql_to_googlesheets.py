...
def main():
    log_msg = f"Creating Google Sheets workbook called '{TITLE}'"
    logger.info(log_msg)
    ...
    try:
        ...
        if isinstance(new_sheet, str):
            log_msg = f"Successfully created sheet {TITLE}!"
            logger.info(log_msg)
        # If we do not get a string back from the create_spreadsheet call
        # then something went wrong. Print the response.
        else:
            log_msg = f"create_spreadsheet did not return a sheet ID. Issue: {new_sheet!s}"
            logger.info(log_msg)

    # If we get an error when trying to create the spreadsheet we print the error.
    except Exception as e:
        log_msg = f"There was a problem creating the Google Sheets workbook! Error: {e!s}"
        logger.info(log_msg)

    logger.info("Querying MYSQL database...")
    query_results = mysql.query(QUERY)

    log_msg = f"Querying complete. Preparing to load data into Google Sheets tab {TAB_LABEL}"
    logger.info(log_msg)
    ...
    log_msg = f"Load into Google Sheets for tab {TAB_LABEL} complete!"
    logger.info(log_msg)
